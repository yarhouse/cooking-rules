...
1. Add `src/app/data/spells.data.ts` with a typed array
2. Add the API route in `server/src/routes/spells.ts`
3. Add three lines to `CookingDataService`: private signal, refresh subject (if mutable), public getter
4. Done — all components use the getter, static and API builds both work
