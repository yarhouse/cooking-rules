...
## Icons: self-hosted via npm

Material Icons are bundled via the `material-icons` npm package — no CDN dependency. This is essential for Electron (offline) and avoids the flash of unstyled icons on first load.

```json
// angular.json — in build.options.styles
"styles": [
...
`index.html` has no `<link>` to fonts.googleapis.com for icons. Roboto (the text font) still loads from Google Fonts — replace with `@fontsource/roboto` if you need full offline support.

To install: `npm install material-icons`
...
For a custom colour, use the [Material Theme Builder](https://m3.material.io/theme-builder) to generate a palette map.
...
These automatically switch between light and dark values when the theme class changes. Use these tokens instead of hardcoded colours.

---

## Pitfalls

**`mat.core()` called in a component:** Only ever call it once in `styles.scss`.

**Using `mat.all-component-themes()` in multiple places:** Use it only in the default `html {}` block. Use `mat.all-component-colors()` (colors only) for the dark/light overrides — it's far smaller.

**Forgetting `color-scheme`:** Without this, native browser controls stay in light mode even when your app is dark.

**Hardcoding colours in component SCSS:** If you write `color: #333` instead of `color: var(--mat-sys-on-surface)`, that component won't respond to the theme toggle.
...
2. Add `node_modules/material-icons/iconfont/material-icons.css` to `angular.json` styles (before `src/styles.scss`)
3. Remove the `Material+Icons` CDN `<link>` from `index.html`
4. Copy `src/styles.scss` in its entirety — change the two palette names
5. Copy `initDarkMode()` and `toggleTheme()` from `src/app/app.ts` — they're self-contained
6. For component overrides, look up the `--mat-<component>-*` token in DevTools and add it to the overrides map
