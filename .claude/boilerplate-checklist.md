...
For full reasoning and pitfall notes behind each step, see the linked pattern guides.
...
cd <project-name>
npm install @angular/material @angular/cdk material-icons
```
...
Remove the `Material+Icons` CDN `<link>` from `index.html` — icons are now self-hosted and work offline in Electron.

Global tokens used in components follow the `--mat-sys-*` naming scheme. Use them instead of hardcoded colours.

See [material-m3-theme.md](patterns/material-m3-theme.md) for the three-layer dark mode cascade and how to find component override tokens in DevTools.
...
`server/src/db.ts` and `server/src/index.ts` are fully generic — only log messages mention the app name. The `PRAGMA user_version` init pattern works for any schema.
...
Create static data files in `src/app/data/*.data.ts`.

See [dual-build-static-api.md](patterns/dual-build-static-api.md) for the `CookingDataService` signal pattern and `ApiService` wrapper.
...
- Add npm scripts: `electron:start`, `dist:mac`, `dist:win`, `build:electron`, `postinstall`
...
Run `npm install` once after adding `electron` — `postinstall` compiles `better-sqlite3` against Electron's Node.
...
7. Add the signal + getter to the data service
...
- [ ] `material-icons` installed; added to `angular.json` styles; CDN link removed from `index.html`
...
- [ ] Data service pattern replicated for your entity types
